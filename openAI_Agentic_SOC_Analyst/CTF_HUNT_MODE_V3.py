...
"""
    ...
    # Ask human to specify log and fields
    print(f"{Fore.LIGHTGREEN_EX}Based on the guidance above, please specify:{Fore.RESET}\n")
    
    try:
        log_name = input(f"{Fore.LIGHTCYAN_EX}Log Name (e.g., DeviceLogonEvents): {Fore.RESET}").strip()
        if not log_name:
            log_name = "DeviceLogonEvents"  # Default
        
        field_names = input(f"{Fore.LIGHTCYAN_EX}Field Names (comma-separated): {Fore.RESET}").strip()
        if not field_names:
            field_names = "TimeGenerated, DeviceName, AccountName"  # Default
        
        print(f"\n{Fore.LIGHTGREEN_EX}✓ Target: {log_name}{Fore.RESET}")
        print(f"{Fore.LIGHTGREEN_EX}✓ Fields: {field_names}{Fore.RESET}\n")
...
def human_kql_entry_stage(bot_guidance):
    ...
    # Show available fields for the suggested table
    log_name = bot_guidance['log_name']
    field_list = AZURE_SCHEMA_REFERENCE.get_field_list(log_name)
    
    print(f"{Fore.LIGHTCYAN_EX}Target Table: {Fore.WHITE}{log_name}{Fore.RESET}")
    print(f"{Fore.LIGHTCYAN_EX}Available Fields: {Fore.WHITE}{', '.join(field_list)}{Fore.RESET}\n")
...
def document_result_stage(flag_intel, session, kql_query, results_csv):
    ...
    print(f"{Fore.WHITE}Document the correct KQL and output for this flag.{Fore.RESET}\n")
    
    # Show current KQL
    print(f"{Fore.LIGHTCYAN_EX}Your KQL Query:{Fore.RESET}")
    print(f"{Fore.LIGHTBLACK_EX}{'─'*70}")
    print(f"{Fore.WHITE}{kql_query}{Fore.RESET}")
    print(f"{Fore.LIGHTBLACK_EX}{'─'*70}{Fore.RESET}\n")
    
    # Parse results to show output
    import io
    df = pd.read_csv(io.StringIO(results_csv))
    
    print(f"{Fore.LIGHTCYAN_EX}Query Output (first 5 rows):{Fore.RESET}")
    print(f"{Fore.LIGHTBLACK_EX}{'─'*70}")
    print(f"{Fore.WHITE}{df.head(5).to_string(index=False)}{Fore.RESET}")
    print(f"{Fore.LIGHTBLACK_EX}{'─'*70}{Fore.RESET}\n")
    
    # Get the answer from human
    print(f"{Fore.LIGHTGREEN_EX}Enter the flag answer:{Fore.RESET}")
    try:
        ...
        # Optional notes
        notes = input(f"{Fore.LIGHTCYAN_EX}Finding notes (optional): {Fore.RESET}").strip()
    ...
    session.capture_flag(
        ...
        notes=notes,
        kql_used=kql_query,
        table_queried=log_name if 'log_name' in locals() else '',
        stage=flag_intel.get('mitre', '').split('-')[0].strip() if '-' in flag_intel.get('mitre', '') else '',
        mitre=flag_intel.get('mitre', ''),
        raw_intel=flag_intel['raw_intel'],
        objective=flag_intel.get('objective', '')
    )
    ...
    print(f"{Fore.LIGHTGREEN_EX}[ENTER]{Fore.RESET} Continue\n")
    input(f"{Fore.LIGHTCYAN_EX}→ {Fore.RESET}")
...
def prompt_next_action(session):
    ...
    try:
        choice = input(f"{Fore.LIGHTGREEN_EX}Select [1-3]: {Fore.RESET}").strip()
